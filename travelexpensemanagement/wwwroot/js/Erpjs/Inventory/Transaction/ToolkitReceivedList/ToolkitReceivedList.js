...
$(document).ready(async function () {
    ...
    TIPagination = Pagination.create({
        ...
        loader: function (params) {
            $.ajax({
                url: '/ToolkitReceivedList/GetAllToolkitIssue',
                type: 'GET',
                dataType: 'json',
                data: {
                ...
                success: function (res) {
                    console.log("Toolkit received list : ", res);
                    params.callback({
                        data: res.data,
        ...
        render: function (docs) {
            const tbody = $('#tblToolkitReceivedList tbody');
            tbody.empty();
            if (!docs.length) {
...
async function checkModificationAllowed(vDate, rowId, vType) {
    checkModificationDays({
        controller: 'ToolkitReceivedList',
        vDate: vDate,
        rowId: rowId,
        vType: vType,
...
function editToolkitIssueEntry(docCode, docType) {
    window.location.href = `/ToolkitReceivedEntry/Index?id=${encodeURIComponent(docCode)}&vtype=${encodeURIComponent(docType)}`;
}

function viewToolkitIssueEntry(docCode, docType) {
    window.location.href = `/ToolkitReceivedEntry/Index?id=${encodeURIComponent(docCode)}&vtype=${encodeURIComponent(docType)}&readOnly=true`;
}

function deleteToolkitIssueEntry(docId, docType) {
    deleteRecordbytype("ToolkitReceivedList", docId, docType, {
        action: "Delete",
        text: "This will permanently delete the Toolkit Received details.",
...
if (btn) {
    btn.addEventListener("click", function (e) {
        e.preventDefault();
        window.location.href = "/ToolkitReceivedList/ExportAllDocs";
    });
}

function showDocumentPopup(docCode, docType) {
    $.ajax({
        url: '/ToolkitReceivedList/PBPEntryDetails',
        type: 'Get',
        dataType: 'json',
