...
async function getDdlData(type) {
    return await $.ajax({
        url: '/ToolkitReceivedEntry/GetDdlList',
        type: 'GET',
        data: { type: type },
        global: false
    });
...
function loadItemList(dropdownId) {
    ...
    ddl.select2({
        ...
        ajax: {
            url: '/ToolkitReceivedEntry/GetItemList',
            dataType: 'json',
            delay: 250,
            global: false,
...
async function GetVNo(vType) {
    try {
        const res = await fetch(`/ToolkitReceivedEntry/GetVNo?vType=${encodeURIComponent(vType)}`);
...
function saveOrUpdate() {
    ...
    $.ajax({
        url: '/ToolkitReceivedEntry/SaveOrUpdate',
        type: 'POST',
        ...
        success: function (res) {
            if (res.success) {
                ...
                setTimeout(() => {
                    window.location.href = '/ToolkitReceivedList/Index';
                }, 1500);
            }
            else {
...
async function getDataById() {
    try {
        const res = await $.ajax({
            url: '/ToolkitReceivedEntry/GetDataById',
            type: 'GET',
            data: {
                vType: rowIdVType,
                vNo: rowId
...
//----Received----
async function GenerateReceivedReport() {
    ...
    var formulaFields = {
        ...
        Parameters: {
            ...
            comp_add1: add1,
            comp_add2: add2,
            RPTNAME: "STORE ITEM RECEIVED FROM EMPLOYEE "
        }
    };
...
async function getGlobalValues() {
    try {
        const response = await $.ajax({
            url: "/ToolkitReceivedEntry/GetGlobalValues",
            type: "GET",
            dataType: "json"
        });
