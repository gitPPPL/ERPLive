	...
	$.ajax({
		...
		success: function (res) {
			...
			if (visitors.length === 0) {
				tbody.append('<tr><td class="text-center text-muted">No visitors found.</td></tr>');
				renderNewPagination();
				return;
			}
			...
			$.each(visitors, function (index, item) {
				...
				tbody.append(`
					<tr class="no-border-input">
						<td class="hidden-col">${item.doC_ID || ''}</td>
						<td>${item.v_NO || ''}</td>
						...
						<td class="action-col">
							...
							<button class="act-btn edit permission-edit" title="Edit" style="cursor:pointer;" id="button_edit" onclick="checkModificationAllowed('${item.v_DATE}', '${item.doC_ID}')">
								<i class="fa fa-edit"></i>
						    </button>

							<button class="act-btn view btn-view" title="View" style="cursor:pointer;" onclick="viewVisitor('${item.doC_ID}')">
								<i class="fa fa-eye"></i>
							</button>

							<button class="act-btn delete permission-delete" title="Delete" style="cursor:pointer;" id="button_delete" onclick="deleteVisitor('${item.doC_ID}')">
