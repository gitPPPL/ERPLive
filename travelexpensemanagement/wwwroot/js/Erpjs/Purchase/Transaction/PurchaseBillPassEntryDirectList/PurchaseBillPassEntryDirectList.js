...
$(document).ready(async function () {
    ...
    PBPPagination = Pagination.create({
        ...
        loader: function (params) {
            $.ajax({
                url: '/PurchaseBillPassEntryDirectList/GetAllPurchaseBillPassEntry',
                type: 'GET',
                dataType: 'json',
                data: {
        ...
        render: function (docs) {
            ...
            tbody.empty();
            if (!docs.length) {
                tbody.append(`<tr><td colspan="19" class="text-center text-muted">No list found.</td></tr>`);
                return;
            }
...
async function checkModificationAllowed(vDate, rowId, vType) {
    ...
    checkModificationDays({
        controller: 'PurchaseBillPassEntryDirectList',
        vDate: vDate,
        rowId: rowId,
        vType: vType,
...
async function GetPurchaseEditStatus(vNo, vType, isApprovalBody) {
    $.ajax({
        url: '/PurchaseBillPassEntryDirectList/GetPurchaseEditStatus',
        type: 'GET',
        data: {
            vType: vType,
            vNo: vNo
...
function editPruchaseBillPassEntry(docCode, docType) {
    window.location.href = `/PurchaseBillPassEntryDirect/Index?id=${encodeURIComponent(docCode)}&vtype=${encodeURIComponent(docType)}`;
}

function viewPruchaseBillPassEntry(docCode, docType) {
    window.location.href = `/PurchaseBillPassEntryDirect/Index?id=${encodeURIComponent(docCode)}&vtype=${encodeURIComponent(docType)}&readOnly=true`;
}

function deletePruchaseBillPassEntry(docId, docType) {
    deleteRecordbytype("PurchaseBillPassEntryDirectList", docId, docType, {
        action: "Delete",
...
if (btn) {
    btn.addEventListener("click", function (e) {
        e.preventDefault();
        window.location.href = "/PurchaseBillPassEntryDirectList/ExportAllDocs";
    });
}

function showDocumentPopup(docCode, docType) {
    $.ajax({
        url: '/PurchaseBillPassEntryDirectList/PBPEntryDetails',
        type: 'Get',
        dataType: 'json',
...
async function checkApprovalBody(vType) {
    try {
        const response = await $.ajax({
            url: '/PurchaseBillPassEntryDirectList/GetApprovalBody',
            type: 'GET',
            data: {
                vType: vType
            }
...
async function checkPurchaseDelete(vNo, vType) {
    ...
    $.ajax({
        url: '/PurchaseBillPassEntryDirectList/GetPurchaseDeleteStatus',
        type: 'GET',
        data: {
            vType: vType,
            vNo: vNo
