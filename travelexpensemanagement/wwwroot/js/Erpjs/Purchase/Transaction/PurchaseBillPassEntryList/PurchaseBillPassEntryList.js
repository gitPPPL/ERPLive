...
$(document).ready(async function () {
    ...
    PBPPagination = Pagination.create({
        ...
        loader: function (params) {
            $.ajax({
                url: '/PurchaseBillPassEntryList/GetAllPurchaseBillPassEntry',
                type: 'GET',
                dataType: 'json',
                data: {
...
async function checkModificationAllowed(vDate, rowId, vType) {
    ...
    checkModificationDays({
        controller: 'PurchaseBillPassEntryList',
        vDate: vDate,
        rowId: rowId,
        vType: vType,
...
async function GetPurchaseEditStatus(vNo, vType, isApprovalBody) {
    $.ajax({
        url: '/PurchaseBillPassEntryList/GetPurchaseEditStatus',
        type: 'GET',
        data: {
            vType: vType,
            vNo: vNo
...
function editPruchaseBillPassEntry(docCode, docType) {
    window.location.href = `/PurchaseBillPassEntry/Index?id=${encodeURIComponent(docCode)}&vtype=${encodeURIComponent(docType)}`;
}

function viewPruchaseBillPassEntry(docCode, docType) {
    window.location.href = `/PurchaseBillPassEntry/Index?id=${encodeURIComponent(docCode)}&vtype=${encodeURIComponent(docType)}&readOnly=true`;
}

function deletePruchaseBillPassEntry(docId, docType) {
    deleteRecordbytype("PurchaseBillPassEntryList", docId, docType, {
        action: "Delete",
...
if (btn) {
    btn.addEventListener("click", function (e) {
        e.preventDefault();
        window.location.href = "/PurchaseBillPassEntryList/ExportAllDocs";
    });
}

function showDocumentPopup(docCode, docType) {
    $.ajax({
        url: '/PurchaseBillPassEntryList/PBPEntryDetails',
        type: 'Get',
        dataType: 'json',
...
async function checkApprovalBody(vType) {
    try {
        const response = await $.ajax({
            url: '/PurchaseBillPassEntryList/GetApprovalBody',
            type: 'GET',
            data: {
                vType: vType
            }
...
async function checkPurchaseDelete(vNo, vType) {
    ...
    $.ajax({
        url: '/PurchaseBillPassEntryList/GetPurchaseDeleteStatus',
        type: 'GET',
        data: {
            vType: vType,
            vNo: vNo
