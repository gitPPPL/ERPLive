...
async function DDLRefREqNo() {
    try {
        const res = await fetch('/PurchaseSauda/DDLRefREqNo');
        const data = await res.json();
        const ddl = $('#ddlREFNO');
        ddl.empty().append('<option value="">-- Select Delivery From --</option>');
        data.forEach(item => {
...
async function fetchDatabyRefNo(RefNo) {
    try {
        const response = await fetch(`/PurchaseSauda/GetDataByRefNo?RefNo=${RefNo}`);
        const data = await response.json();

        console.log("data", data);

        $('#ddlPartyName').val(data.refnodata.partyCode);
        $('#txtAddress1').val(data.refnodata.address1);
        $('#txtAddress2').val(data.refnodata.address2);
        $('#txtAddress3').val(data.refnodata.address3);
        $('#txtContactNo').val(data.refnodata.phone);

        $('#txtStation').val(data.refnodata.cityCode);

        $('#txtCountry').val(data.refnodata.countrY_CODE);
        $('#txtDeliveryFrom').val(data.refnodata.deliveryTo);
        $('#ddlItemName').val(data.refnodata.itemCode);
        $('#numTrucks').val(data.refnodata.truck);
        $('#numWeight').val(data.refnodata.qty);
        $('#numRate').val(data.refnodata.rate);
        $('#numDiscount').val(data.refnodata.disc);
        $('#numFreightRate').val(data.refnodata.f_rate);
        $('#numNetRate').val(data.refnodata.netRate);

        if (!data.refnodata.type) {
            $('#ddlItemType').prop('selectedIndex', 0);
        } else {
            $('#ddlItemType').val(data.refnodata.type);
        }


        if (!data.refnodata.f_term) {
            $('#ddlFreightTerm').prop('selectedIndex', 0);
        }
        else {
            $('#ddlFreightTerm').val(data.refnodata.f_term);
        }

        if (!data.refnodata.t_term) {
            $('#cmbtaxterm').prop('selectedIndex', 0);
        } else {

            $('#cmbtaxterm').val(data.refnodata.t_term);

        }

        $('#ddlPaymentTerm').val(data.refnodata.payterM_CODE);
        $('#txtDeliveryTerm').val(data.refnodata.d_term);
        $('#txtRemarks').val(data.refnodata.remark);
        $('#numDiscount').val(data.refnodata.disc);
        $('#ddlPurchaseThrough').val(data.refnodata.deaL_THROUGHCode);
