...
function fillPurchaseDataDetail(selectedItemCode)
{
    $.ajax({
        ...
        success: function (response) {
            if (response.status) {
                let $tableId = $('#quotationapprovalhistory tbody'); 
                $tableId.empty();
                $.each(response.data, function (index, item) {
                    var rowDt = `
                    <tr>
...
function FilterDataList() {
    ...
    console.log(
        "From Checked =", $('#chkFromDate').is(':checked'),
        "From Value =", $('#chkFromDate').val(),
        "To Checked =", $('#chkToDate').is(':checked'),
        "To Value =", $('#chkToDate').val()
    );
    ...
    const itemList = selectedItems.map(x => x.id);
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            if (
                ...
                // Clear dropdown after fill Added By Sumesh
...
function SaveData(saveDt) {
    $.ajax({
        ...
        success: function (response) {
            console.log(response.status, JSON.stringify(saveDt));
            if (response?.status) {
...
function getItemDetailsFromTable() {
    ...
    $('#tblItemDetailsQR tbody tr').each(function () {
        ...
        const detail = {
            ...
            FAPROV_STATUS: getStringOrNull(selectedValue),
            FAPROV_REMARKS: getStringOrNull(getText('FAPROV_REMARKS')),
...
function GetItemList() {
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            //console.log("CONVERTED", dropdownData[0]);
...
function fillItemDetailsTable(data, ApprvlStage = 1, append = false) {  
    ...
    const row = `
        ...
        ${finalApprovalStatusTd}
        <td style="display:none;" id="row-${index}-FAPROV_REMARKS">${item.FAPROV_REMARKS}</td>
...
function validateData() {
    ...
    for (let i = 0; i < $rows.length; i++) {
        ...
        const remarks =
            $(`#row-${rowIndex}-FAPROV_REMARKS`).text().trim();
