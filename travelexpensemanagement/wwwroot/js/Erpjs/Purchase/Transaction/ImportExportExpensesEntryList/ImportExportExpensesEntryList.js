...
$(document).ready(async function () {
    ...
    IEEEPagination = Pagination.create({
        pageSize: 10,
        paginationContainer: '#pageNumbers',
        infoContainer: '#pageInfoText',
        loader: function (params) {
            $.ajax({
                url: '/ImportExportExpensesEntryList/GetAllPurchaseBillPassEntry',
                type: 'GET',
                dataType: 'json',
                data: {
...
async function checkModificationAllowed(vDate, rowId, vType) {
    ...
    checkModificationDays({
        controller: 'ImportExportExpensesEntryList',
        vDate: vDate,
        rowId: rowId,
        vType: vType,
...
async function GetPurchaseEditStatus(vNo, vType, isApprovalBody) {
    $.ajax({
        url: '/ImportExportExpensesEntryList/GetPurchaseEditStatus',
        type: 'GET',
        data: {
            vType: vType,
            vNo: vNo
...
function editPruchaseBillPassEntry(docCode, docType) {
    window.location.href = `/ImportExportExpensesEntry/Index?id=${encodeURIComponent(docCode)}&vtype=${encodeURIComponent(docType)}`;
}

function viewPruchaseBillPassEntry(docCode, docType) {
    window.location.href = `/ImportExportExpensesEntry/Index?id=${encodeURIComponent(docCode)}&vtype=${encodeURIComponent(docType)}&readOnly=true`;
}

function deletePruchaseBillPassEntry(docId, docType) {
    deleteRecordbytype("ImportExportExpensesEntryList", docId, docType, {
        action: "Delete",
        text: "This will permanently delete the Purchase Bill Pass Entry details.",
        successCallback: IEEEPagination.load
    });
}
...
if (btn) {
    btn.addEventListener("click", function (e) {
        e.preventDefault();
        window.location.href = "/ImportExportExpensesEntryList/ExportAllDocs";
    });
}

function showDocumentPopup(docCode, docType) {
    $.ajax({
        url: '/ImportExportExpensesEntryList/PBPEntryDetails',
        type: 'Get',
        dataType: 'json',
...
async function checkApprovalBody(vType) {
    try {
        const response = await $.ajax({
            url: '/ImportExportExpensesEntryList/GetApprovalBody',
            type: 'GET',
            data: {
                vType: vType
            }
...
async function checkPurchaseDelete(vNo, vType) {
    ...
    $.ajax({
        url: '/ImportExportExpensesEntryList/GetPurchaseDeleteStatus',
        type: 'GET',
        data: {
            vType: vType,
            vNo: vNo
