...
$(document).ready(function () {
    ...
    $('#VisitorEntryForm').on('input change', 'input, select, textarea', function () {
        if (ignoreDirty) return;
        if (!formState.isReadOnlyFromUrl) {
            formState.isFormDirty = true;
        }
    });
...
function returnBack(url) {
    ...
    // ONLY safe case → no popup
    if (formState.isSaved || formState.isReadOnlyFromUrl) {
        window.location.href = url;
        return;
    }

    // ALL other cases → popup
    handleBack(url);
}

//function returnBack(url) {

//    if (formState.isReadOnlyFromUrl || formState.isSaved) {
//        window.location.href = url;
//        return;
//    }
...
//    handleBack(url); // only dirty case
//}
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            if (response.success) {
                ...
                formState.isFormDirty = false;
                if (response.message.includes("Saved")) {

                    if (response.docId) {
                        $('#DOCID').val(response.docId);
                        window.location.href = `/VisitorEntry?docId=${response.docId}&readOnly=true`;
                        return;
                    }
...
function loadVisitorEntry(docId) {
    ...
    $.ajax({
        ...
        success: function (res) {
            ...
            {
                const imageType = visitor.filE_NAME && visitor.filE_NAME.toLowerCase().endsWith('.png') ? 'png' : 'jpeg';
                $('#previewImage').attr('src', 'data:image/' + imageType + ';base64,' + base64Image).css('display', 'inline-block');
            ...
            formState.isFormDirty = false;
            formState.isSaved = false;
            ignoreDirty = false;
            formState.isReadOnlyFromUrl = isReadOnly;
        },
