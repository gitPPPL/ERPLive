    ...
    public class SalesOrderRepository : ISalesOrderRepository
    {
        ...
        public async Task<RepositoryResponseData<object>> GetPartyAddress(int code, int addressId)
        {
            try
            {
                ...
                return new RepositoryResponseData<object> { status = true, data = PartyAddList };
            }
            catch (Exception ex)
            {
                return new RepositoryResponseData<object> { status = true, message = "data load failed" };
            }
        }

        public async Task<RepositoryResponseData<object>> GetSaudaDataList(string saudaNo)
        {
            try
            {
                var gv = _globalVariableService.GetGlobalVariables();
                ...
                return new RepositoryResponseData<object> { status = true, data = dataList };
            }
            catch (Exception ex)
            {
                return new RepositoryResponseData<object> { status = false, message = "data load failed" };
            }
        }

        public async Task<RepositoryResponseData<object>> GetIssueDetails(string issueNo)
        {
            var gv = _globalVariableService.GetGlobalVariables();
        ...
        public async Task<RepositoryResponseData<object>> GetPackingDetail(int vno, string packingNo, string doctype)
        {
            try
            {
                var gv = _globalVariableService.GetGlobalVariables();
                ...
                return new RepositoryResponseData<object>
                {
                    ...
                    {
                        details = result,
                        totalQty = result.Sum(x => x.QTY),
                        totalNos = result.Sum(x => x.NOS)
                    }
                };
            }
            catch (Exception ex)
            {
                return new RepositoryResponseData<object> { status = false, message = "Data load failed: " + ex.Message };
            }
        }

        public async Task<RepositoryResponseData<object>> GetLinkedOrders(int saudaNo)
        {
            var gv = _globalVariableService.GetGlobalVariables();
        ...
        public async Task<RepositoryResponseData<object>> GetSaudaRate(GetSaudaRateRequest request)
        {
            ...
            try
            {
                ...
                if (saudaRate == 0)
                {
                    return new RepositoryResponseData<object> { status = false, message = "Sauda Rate is blank" };
                }
                ...
                // 5. RETURN JSON
                return new RepositoryResponseData<object> { status = true, data = new { saudaNo = request.SaudaNo, saudaRate = saudaRate, items = request.Items } };
            }
            catch (Exception ex)
            {
        ...
        public async Task<RepositoryResponseData<decimal>> GetPackAmount(int itemCode)
        {
            var gv = _globalVariableService.GetGlobalVariables();
            try
            {
            ...
            {
                return new RepositoryResponseData<decimal> { status = false, message = ex.Message };
            }
        }

        public async Task<RepositoryResponseData<List<CalculateAmtItemModel>>> GetCalculateAmtData(int itemCode)
        {
            var gv = _globalVariableService.GetGlobalVariables();
            ...
            try
            {
                ...
                return new RepositoryResponseData<List<CalculateAmtItemModel>> { status = true, data = result };
            }
            catch (Exception ex)
            {
                return new RepositoryResponseData<List<CalculateAmtItemModel>> { status = false, message = ex.Message };
            }
        }

        public async Task<RepositoryResponse> ValidateData(ValidateDataModel model)
        {
            var gv = _globalVariableService.GetGlobalVariables();
                ...
                using (var cmd = new SqlCommand(query, con))
                {
                    ...
                    using (var reader = await cmd.ExecuteReaderAsync())
                    {
                        while (await reader.ReadAsync())
                        {
                            ...
                            {
                                return new RepositoryResponse { status = false, message = "Sold item must exist in Order." };
                            }
                        }
                    }
                }
        ...
        public async Task<RepositoryResponseData<object>> GetPurchaseOrderRecordsById(int id, string vType)
        {
            try
            {
                ...
                {
                    return new RepositoryResponseData<object> { status = false, message = "Invalid ID" };
                }
                ...
                var parametersExistingItems = new Dictionary<string, object>
                {
                    { "@COMP_CODE", int.Parse(userSession.PubCompCode) },
                    { "@BRANCH_CODE", userSession.PubBranchCode },
                    { "@V_NO", id },
                    { "@Action", "ExistingSalesOrderItems" }
                };

                var header = await _dbHelper.GetJsonFromProcedureAsync("[dbo].[sp_SalesOrder]", parametersHeader);
                var detail = await _dbHelper.GetJsonFromProcedureAsync("[dbo].[sp_SalesOrder]", parametersDetail);
                var existingItems = await _dbHelper.GetJsonFromProcedureAsync("[dbo].[sp_SalesOrder]", parametersExistingItems);
        ...
        public async Task<RepositoryResponseData<object>> SaveOrUpdateSalesOrder(PurchaseOrder POmodel, string doctype)
        {
            if (POmodel == null)
                return new RepositoryResponseData<object> { status = false, message = " data save failed." };
            try
            {
                using (var con = _dbcontext.GetErpConnection())
                {
                    ...
                    {
                        return new RepositoryResponseData<object> { status = true, message = validation.Message, data = new { isWarning = true } };
                    }
                    ...
                    using (var transaction = con.BeginTransaction())
                    {
                        try
                        {
                            ...
                            using (SqlCommand cmd = new SqlCommand("[dbo].[sp_SalesOrder]", con, transaction))
                            {
                        ...
                        catch (Exception ex)
                        {
                            ...
                            return new RepositoryResponseData<object> { status = false, message = "Transaction failed: " + ex.Message };
                        }
                    }
                }
            }
        ...
        public async Task<(bool IsApprovalRequired, string Message)> CheckSaudaApproval(PurchaseOrder model)
        {
            using var con = _dbcontext.GetErpConnection();
            await con.OpenAsync();
            ...
            {
                approvalCmd.Parameters.AddWithValue("@USER_CODE", gv.PubUserId);
                approvalCmd.Parameters.AddWithValue("@DOC_CODE", "SORD");
                approvalCmd.Parameters.AddWithValue("@COMP_CODE", gv.PubCompCode);
                ...
                if (await reader.ReadAsync())
                {
                    isFinalApprovalBody = reader["APPROV_USER"] != DBNull.Value && reader["APPROV_USER"].ToString() == "FINAL";
                }
            }
            ...
            if (gs.pubDefSSINSO == "Yes")
            {
                if (model.SaudaNo > 0)
                {
                    string sql = @"SELECT TOP 1 ITEM_CODE, RATE FROM SAUDA WHERE V_TYPE=@V_TYPE AND V_NO=@V_NO AND COMP_CODE=@COMP_CODE AND BRANCH_CODE=@BRANCH_CODE";
                ...
                foreach (var item in model.ItemRecords)
                {
                    ...
                    if (existResult != null)
                    {
                        string rateSql = @"SELECT TOP 1 ISNULL(RATE,0) FROM RMDISC_MAST WHERE ITEM_CODE=@ITEM_CODE AND COMP_CODE=@COMP_CODE AND SAUDA_ITEM=@SAUDA_ITEM AND EFF_DATE<@EFF_DATE
                        ...
                        if (result == null || result == DBNull.Value)
                            fg = 1;
                        else if (item.Rate > saudaRate + Convert.ToDecimal(result))
                            fg = 1;
                    }
                    else
                    {
                        string saudaCheckSql = @"SELECT TOP 1 1 FROM SAUDA WHERE V_TYPE='SAUD' AND V_NO=@V_NO AND PARTY_CODE=@PARTY_CODE AND ITEM_CODE=@ITEM_CODE AND RATE=@RATE
                        ...
                        if (await saudaCheckCmd.ExecuteScalarAsync() == null)
                            fg = 1;
                    }
                }
            }
            ...
            {
                return (true, $"Item Name/Party Name/Rate not matched with data in SAUDA of Sauda No:{model.SaudaNo}, Approval Required.");
            }

            return (false, "");
        }

        public RepositoryResponseData<object> GetOrderAdjustment(int ordNo)
        {
            try
            {
                ...
                string qry = @"SELECT a.v_no AS BillNo, FORMAT(a.v_Date, 'dd/MM/yyyy') AS BillDate, IIF(ISNULL(b.SHORTNAME, '') = '', b.Name, b.SHORTNAME) AS ItemName,
                                SUM(a.qty) AS Quantity FROM SALE2 a LEFT JOIN item_mast b ON a.ITEM_CODE = b.CODE AND a.COMP_CODE = b.COMP_CODE
                                WHERE a.Status <> 2 AND a.V_type <> 'SART' AND a.ORD_NO = @ORD_NO AND a.comp_code = @COMP_CODE AND a.branch_code = @BRANCH_CODE
                                GROUP BY a.v_no, a.v_Date, b.SHORTNAME, b.NAME ORDER BY a.V_NO";

                using (SqlConnection con = _dbcontext.GetErpConnection())
                using (SqlCommand cmd = new SqlCommand(qry, con))
                {
                    cmd.Parameters.AddWithValue("@ORD_NO", ordNo);
                    cmd.Parameters.AddWithValue("@COMP_CODE", gv.PubCompCode);
                    cmd.Parameters.AddWithValue("@BRANCH_CODE", gv.PubBranchCode);
        ...
        public RepositoryResponse SaveDispatchDetails(List<DispatchDeliveryPlaning> dispatch, string doctype)
        {
            var gv = _globalVariableService.GetGlobalVariables();

            if (dispatch == null || dispatch.Count == 0)
                return new RepositoryResponse { status = false, message = "Invalid delivery plan data." };

            var validation = ValidateDispatchDetails(dispatch);
            ...
            try
            {
                ...
                try
                {
                    int vNo = dispatch[0].v_no;

                    string deleteSql = @"DELETE FROM ORDER_DELPLAN WHERE COMP_CODE = @COMP_CODE AND V_NO = @V_NO AND BRANCH_CODE = @BRANCH_CODE
                                        AND YEAR_CODE = @YEAR_CODE AND V_TYPE = @V_TYPE";

                    using (var deleteCmd = new SqlCommand(deleteSql, conn, transaction))
                    {
                        deleteCmd.Parameters.AddWithValue("@COMP_CODE", gv.PubCompCode);
                        deleteCmd.Parameters.AddWithValue("@V_NO", vNo);
                        ...
                        deleteCmd.Parameters.AddWithValue("@V_TYPE", doctype);
                        deleteCmd.ExecuteNonQuery();
                    }

                    string qry = @"INSERT INTO ORDER_DELPLAN (YEAR_CODE, COMP_CODE, BRANCH_CODE, V_TYPE, V_NO, DOC_ID, V_DATE, SNO, ITEM_NAME, ITEM_CODE, 
                                    DELIVERY_DATE, QTY, BAL_QTY, UUSER, UDATE, EUSER, EDATE, AED, WSID, LIP, LID, Remarks)
                                    VALUES (@YEAR_CODE, @COMP_CODE, @BRANCH_CODE, @V_TYPE, @V_NO, @DOC_ID, @V_DATE, @SNO, @ITEM_NAME, @ITEM_CODE, @DELIVERY_DATE, 
                                    @QTY, @BAL_QTY, @UUSER, @UDATE, @EUSER, @EDATE, @AED, @WSID, @LIP, @LID, @Remarks)";

                    int sno = 1;

                    foreach (var item in dispatch)
                    {
                        using var cmd = new SqlCommand(qry, conn, transaction);
                        ...
                        cmd.Parameters.AddWithValue("@DOC_ID", doctype + item.v_no);
                        ...
                        cmd.Parameters.AddWithValue("@ITEM_CODE", item.ItemCode);
                        cmd.Parameters.AddWithValue("@DELIVERY_DATE", item.DeliveryDate);
                        cmd.Parameters.AddWithValue("@QTY", item.Qty);
                        cmd.Parameters.AddWithValue("@BAL_QTY", DBNull.Value);
                    ...
                    return new RepositoryResponse { status = true, message = "Delivery plan saved successfully." };
                }
                catch (Exception ex)
                {
                    transaction.Rollback();

                    return new RepositoryResponse { status = false, message = $"Save failed: {ex.Message}" };
                }
            }
            catch (Exception ex)
            {
        ...
        private (bool status, string message) ValidateDispatchDetails(List<DispatchDeliveryPlaning> dispatch)
        {
            if (dispatch == null || dispatch.Count == 0)
                return (false, "No Record in grid to save.");
            ...
            try
            {
                ...
                string sql = @"SELECT ITEM_CODE, ISNULL(SUM(QTY), 0) AS QTY FROM ORDER2 WHERE V_TYPE = 'SORD' AND V_NO = @V_NO AND COMP_CODE = @COMP_CODE
                                AND BRANCH_CODE = @BRANCH_CODE AND YEAR_CODE = @YEAR_CODE GROUP BY ITEM_CODE";

                var orderItems = new Dictionary<int, decimal>();
                ...
                using (var cmd = new SqlCommand(sql, conn))
                {
                    ...
                    while (reader.Read())
                    {
                        orderItems[Convert.ToInt32(reader["ITEM_CODE"])] = Convert.ToDecimal(reader["QTY"]);
                    }
                }

                // Total Qty validation
                decimal orderTotalQty = orderItems.Values.Sum();
                decimal planTotalQty = dispatch.Sum(x => x.Qty);

                if (planTotalQty != orderTotalQty)
                {
                    return (false, $"Qty not matched. Order Qty: {orderTotalQty},<br>Delivery Plan Qty: {planTotalQty}.");
                }

                // Item-wise Qty validation
                foreach (var item in orderItems)
                {
                    decimal planQty = dispatch
                        .Where(x => x.ItemCode == item.Key)
                        .Sum(x => x.Qty);

                    if (planQty != item.Value)
                    {
                        return (false, $"Qty not matched of Item Code => {item.Key}.<br>Order Qty: {item.Value}, Delivery Plan Qty: {planQty}.");
                    }
                }
        ...
        public RepositoryResponseData<List<DispatchDeliveryPlaning>> GetDispatchDeliveryPlan(int vNo, string doctype)
        {
            try
            {
                var gv = _globalVariableService.GetGlobalVariables();
                var list = new List<DispatchDeliveryPlaning>();

                string sql = @"SELECT ITEM_CODE, ITEM_NAME, DELIVERY_DATE, QTY, Remarks
                       FROM ORDER_DELPLAN
                ...
                while (reader.Read())
                {
                    list.Add(new DispatchDeliveryPlaning
                    {
                ...
                return new RepositoryResponseData<List<DispatchDeliveryPlaning>> { status = true, data = list };
            }
            catch (Exception ex)
            {
                return new RepositoryResponseData<List<DispatchDeliveryPlaning>> { status = false, message = ex.Message };
            }
        }
    }
}
