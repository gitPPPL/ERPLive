...
{
    public class TransitEntryListRepository : ITransitEntryListRepository
    {
        private readonly DataBaseConnection _dbConnection;
        private readonly GlobalVariableService _globalVariableService;
        private readonly travelexpensemanagement.LogService.LogService _logService;
        public TransitEntryListRepository(DataBaseConnection dbConnection, GlobalVariableService globalVariableService, LogService.LogService logService)
        {
        ...
        public async Task<RepositoryResponse> DeleteById(int docId, string docType)
        {
            ...
            catch (Exception ex)
            {
                ...
                response.message = "Error deleting Transit Entry ."+ ex.Message;
                return response;
            }
        }
        public async Task<RepositoryResponseData<TransitEntryModel>> GetById(int code, string vtype)
        {
            var getdata = _globalVariableService.GetGlobalVariables();
            TransitEntryModel TransitEntryModel = null;
            var response = new RepositoryResponseData<TransitEntryModel>();
            try
            {
                using (SqlConnection con = _dbConnection.GetErpConnection())
                {
        ...
        public async Task<RepositoryResponseList<TransitEntryModel>> GetList(string searchTerm = "", int pageNumber = 1, int pageSize = 10)
        {
            ...
            var response = new RepositoryResponseList<TransitEntryModel>();
            int totalCount = 0;
