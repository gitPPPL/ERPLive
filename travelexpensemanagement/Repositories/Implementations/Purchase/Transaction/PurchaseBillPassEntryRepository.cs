...
namespace travelexpensemanagement.Repositories.Implementations.Purchase.Transaction
{
    public class PurchaseBillPassEntryRepository : IPurchaseBillPassEntryRepository
    {
        ...
        //===============Copy From====================
        private static readonly Dictionary<string, string> ModalMap = new()
        {
            ["PORD"] = "#purchaseorderModal",
            ["RCPT"] = "#RMPurchaseModal",
            ["RCPI"] = "#ImportPurchaseModal",
            ["BFRC"] = "#BFRCModal",
            ["SRPU"] = "#SRPUModal"
        };

        private string GetModalId(string docTypeCode) =>
            ModalMap.GetValueOrDefault(docTypeCode, "");
