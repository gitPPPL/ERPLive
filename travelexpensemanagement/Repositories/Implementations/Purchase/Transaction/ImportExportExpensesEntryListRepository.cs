...
{
    public class ImportExportExpensesEntryListRepository : IImportExportExpensesEntryListRepository
    {
        private readonly GlobalVariableService _globalVariableService;
        private readonly DataBaseConnection _dbConnection;

        public ImportExportExpensesEntryListRepository(GlobalVariableService globalVariableService, DataBaseConnection dbConnection)
        {
            ...
            try
            {
                using (SqlConnection conn = _dbConnection.GetErpConnection())
                {
                    using (SqlCommand cmd = new SqlCommand("sp_PurchaseBillPassEntryDirect", conn))
                    {
                        ...
                        cmd.Parameters.AddWithValue("@Doctype", "PurchaseExpenses");
        ...
        public PurchaseDeleteStatus GetPurchaseDeleteStatus(string vType, int vNo)
        {
            ...
            string qry = $@"select 1 from APPROVAL_STATUS where v_type=@V_TYPE and v_NO=@V_NO and comp_code=@comp_code
                            and branch_code=@BRANCH_CODE and year_code=@year_code and status='OPEN'";
            ...
            using (SqlCommand cmd = new SqlCommand(qry, con))
            {
                ...
                if (con.State != ConnectionState.Open)
                    con.Open();
                var appStatus = cmd.ExecuteScalar();
                result.IsApprovalInProcess = Convert.ToInt32(appStatus) == 1;
                //using (SqlDataReader dr = cmd.ExecuteReader())
                //{
                //    if (dr.Read())
                //    {
