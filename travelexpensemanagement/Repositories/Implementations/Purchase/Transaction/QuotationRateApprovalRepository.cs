...
{
    public class QuotationRateApprovalRepository : IQuotationRateApprovalRepository
    {
        ...
        public QuotationRateApprovalRepository(DataBaseConnection dbcontext, DbHelper dbHelper, GlobalVariableService globalValue, ModuleService.ModuleService moduleService, DropdownService dropdownService, GlobalValidationdate globalValidationdate)
        {
            _dbHelper = dbHelper;
        ...
        public async Task<object> GetQuotRtApvrlDetailsById(string id)
        {
            try
            {
                ...
                using (SqlConnection con = _dbcontext.GetErpConnection())
                {
                    ...
                    string query = @"SELECT FILE_NAME, FILE_TYPE, IMG_FILE
                             FROM IMG_TABLE
                ...
                bool isApprovalBody = await IsApprovalUser("STAP");
                bool isFinalApprovalBody = await IsFinalApprovalUser("STAP");
                return new
                {
                    status = true,
                    detail = result,
                    attachment = attachments,
                    isApprovalUser = isApprovalBody,
                    isFinalApprovalUser = isFinalApprovalBody
                };
            }
            catch (Exception ex)
            {
        ...
        public async Task<object> GetFilterItemdetails(FilterItemload filtrItmModel)
        {
            try
            {
        ...
        public async Task<object> SaveOrUpdateQuotRateApproval(QuotationRateApproval equotmodel)
        {
            if (equotmodel == null)
                return new
                {
                    ...
                    using (var transaction = con.BeginTransaction())
                    {
                        ...
                        try
                        {
                            ...
                            return new
                            {
                                status = success,
                                message = success
                                    ? "Data save/update successfully."
                                    : "Failed to save or update some employee details."
                            };
                        }
                        catch (Exception ex)
                        {
        ...
        private async Task<bool> IsApprovalUser(string docType)
        {
            var user = _globalValue.GetGlobalVariables();
            ...
            cmd.Parameters.AddWithValue("@USER_CODE", user.PubUserId);
            cmd.Parameters.AddWithValue("@DOC_CODE", docType);
            cmd.Parameters.AddWithValue("@COMP_CODE", user.PubCompCode);

            return await cmd.ExecuteScalarAsync() != null;
        }

        private async Task<bool> IsFinalApprovalUser(string docType)
        {
            var usersessionDt = _globalValue.GetGlobalVariables();
            ...
            using (var con = _dbcontext.GetErpConnection())
            {
                ...
                AND DOC_CODE = @DOC_CODE
                AND COMP_CODE = @COMP_CODE
                AND APPROV_USER = 'FINAL'";

                using (SqlCommand cmd = new SqlCommand(query, con))
                {
                    cmd.Parameters.AddWithValue("@USER_CODE", usersessionDt.PubUserId);
                    cmd.Parameters.AddWithValue("@DOC_CODE", docType);
                    cmd.Parameters.AddWithValue("@COMP_CODE", usersessionDt.PubCompCode);
