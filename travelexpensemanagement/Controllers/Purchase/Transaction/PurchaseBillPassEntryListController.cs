...
{
    [SessionAuthorize]
    public class PurchaseBillPassEntryListController : Controller
    {
        private readonly GlobalVariableService _globalVariableService;
        private readonly DbHelper _dbHelper;
        private readonly GlobalExcelExport _excel;
        private readonly IPurchaseBillPassEntryListRepository _IPBPListRepository;
        private readonly GlobalValidationdate _globalValidationdate;

        public PurchaseBillPassEntryListController(GlobalVariableService globalVariableService,
            DbHelper dbHelper, GlobalExcelExport excel, IPurchaseBillPassEntryListRepository IPBPListRepository,
            GlobalValidationdate globalValidationdate)
        {
            _globalVariableService = globalVariableService;
            _dbHelper = dbHelper;
            _excel = excel;
            _IPBPListRepository = IPBPListRepository;
            _globalValidationdate = globalValidationdate;
        }
        public IActionResult Index()
        {
            return View("~/Views/Purchase/Transaction/PurchaseBillPassEntryList/Index.cshtml");
        }
        ...
        public IActionResult GetAllPurchaseBillPassEntry(string searchTerm = "", int pageNumber = 1, int pageSize = 10)
        {
            try
            {
                var result = _IPBPListRepository.GetAllPurchaseBillPassEntry(searchTerm, pageNumber, pageSize);
                if(result.data != null)
                {
        ...
        public IActionResult ExportAllDocs()
        {
            try
            {
                ...
                var fileBytes = _excel.ExportToExcel("sp_PurchaseBillPassEntryDirect", "Purchase Bill Pass", parameters);

                return File(
                    fileBytes,
                    "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                    $"PurchaseBillPass_{DateTime.Now:ddMMyyyy}.xlsx"
                );
            }
            catch (Exception ex)
            {
        ...
        public JsonResult Delete(int vNo, string docType)
        {
            ...
            var result = _IPBPListRepository.DeletePurchaseBillPass(vNo, docType);
        ...
        public IActionResult GetPurchaseEditStatus(string vType, int vNo)
        {
            try
            {
                ...
                var result = _IPBPListRepository.GetPurchaseEditStatus(vType, vNo);
        ...
        public IActionResult GetPurchaseDeleteStatus(string vType, int vNo)
        {
            try
            {
                var result = _IPBPListRepository.GetPurchaseDeleteStatus(vType, vNo);
