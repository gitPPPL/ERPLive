...
namespace travelexpensemanagement.Controllers.Purchase.Transaction
{
    public class ImportPaymentEntryController : Controller
    {
        ...
        public ImportPaymentEntryController(DataBaseConnection dbConnection, DbHelper dbHelper, GlobalVariableService globalVariableService, ModuleService.ModuleService moduleService, GlobalValidationdate globalValidationdate, travelexpensemanagement.LogService.LogService logService, travelexpensemanagement.Common.DropdownService.DropdownService dropdownService, IImportPaymentEntryRepository importPaymentEntryRepository)
        {
            ...
            _importPaymentEntryRepository = importPaymentEntryRepository;
        }
        ...
        public IActionResult GetDropdown(string type, string term = "")
        {
            ...
            switch (type)
            {
                ...
                case "PartyDetail":
                    return Json(_dropdownService.GetDropdownList(@"
                select code, name from SUBGROUP_MAST order by name "));
                default:
        ...
        {
            var gv = _globalVariableService.GetGlobalVariables();
            var model = new ImportPaymentEntryModel.PartyDetailsModel();
        ...
        public JsonResult GenerateVNo(string vType)
        {
            ...
            try
            {
                ...
                using (SqlConnection con = _dbConnection.GetErpConnection())
                {
                    ...
                    string query = @"
                    SELECT ISNULL(
                        MAX(CAST(RIGHT(CAST(V_NO AS VARCHAR(20)), 5) AS INT)), 
                        0
                    ) + 1
                    FROM IMPORT_PAY1    
                    WHERE V_TYPE = @VType
        ...
        //[HttpPost]
        //public IActionResult SaveImportPaymentEntry([FromBody] ImportPaymentEntryModel.SaveImportPaymentEntry model)
        //{
        //    var gv = _globalVariableService.GetGlobalVariables();
        ...
        [HttpPost]
        public async Task<IActionResult> SaveImportPaymentEntry([FromBody] ImportPaymentEntryModel.SaveImportPaymentEntry model)
        {
            var result = await _importPaymentEntryRepository.SaveImportPaymentEntryAsync(model);
        ...
        public async Task<IActionResult> LoadEditData(string docId)
        {
            try
            {
                ...
                var result = await _importPaymentEntryRepository.LoadEditDataAsync(docId);
        ...
        //        return Json(new { success = true, header, footer });
        //    }
        //    catch (Exception ex)
        //    {
        ...
        public async Task<IActionResult> CheckValidDate([FromBody] JsonElement data)
        {
            ...
            var result = await _globalValidationdate.CheckValidDate("IMPORT_PAY1", vdate, vtype, vno);
            return Ok(result);
        }
