...
namespace travelexpensemanagement.Controllers.Purchase.Transaction
{
    ...
    public class PurchaseBillPassEntryController : Controller
    {
        ...
        public PurchaseBillPassEntryController(DataBaseConnection dbConnection, GlobalVariableService globalVariableService, DbHelper dbHelper, GlobalValidationdate globalValidationdate, IPurchaseBillPassEntryRepository purchaseBillPassEntry)
        {
        ...
        private string BuildListQuery(string type, string vType, int shipFromCode, int cCode)
        {
            ...
            return type switch
            {
                "doctype" => "SELECT CODE as Value, NAME as Text FROM DOCTYPE_MAST WHERE DOCTYPE in ('PurchaseInvoice','Service Order') ORDER BY NAME",
                ...
                "drcrbyvtype" => $@"select SUBGROUP_MAST.code as Value, SUBGROUP_MAST.name as Text, SUBGROUP_MAST.ADD1, SUBGROUP_MAST.ADD2, 
                ...
                "currency" => $@"SELECT CODE as Value, SHORTNAME as Text FROM CURRENCY_MAST ORDER BY code",
        ...
        public IActionResult GetAddressByBillToParty(int code, int addressId)
        {
            try
            {
                var addressDetails = _purchaseBillPassEntry.GetAddByParty(code, addressId);
        ...
        public IActionResult ValidateMRN(string mrnTypeNo, string vType, int vNo)
        {
            try
            {
                var result = _purchaseBillPassEntry.ValidateMRN(mrnTypeNo, vType, vNo);
                if (!result.status)
                {
        ...
        public async Task<IActionResult> GetPurchaseDetailsByMRN(string vType, int vNo)
        {
            try
            {
                var model = await _purchaseBillPassEntry.GetPurchaseDetailsByMRN(vType, vNo);
                if(model == null)
                {
        ...
        public IActionResult GetPurchaseItemsByMRN(string vType, int vNo)
        {
            try
            {
                var result = _purchaseBillPassEntry.GetPurchaseItemsByMRN(vType, vNo);
        ...
        public JsonResult GetItemOrderRatesByPO(string poType, int poNo, int itemCode)
        {
            try
            {
                ...
                decimal landRate = 0;
                decimal rate = 0;
                var result = _purchaseBillPassEntry.GetItemOrderRatesByPO(poType, poNo, itemCode);
        ...
        public async Task<JsonResult> GetPackOnBasic(int code)
        {
            try
            {
                var result = await _purchaseBillPassEntry.GetPackOnBasic(code);
        ...
        [HttpGet]
        public async Task<JsonResult> GetHsnCodeAndQty(int itemCode, string poType, int poNo)
        {
            try
            {
                var result = await _purchaseBillPassEntry.GetHsnCodeAndQty(itemCode, poType, poNo);
                if (!result.status)
                {
                    return Json(new { success = false, message = result.message });
                }
                int hsnCode = result.data.HsnCode;
                decimal qty = result.data.Qty;
                return Json(new {success = result.status, data = new { hsnCode, qty }});
            }
            catch (Exception ex)
            {
                return Json(new { success = false, message = ex.Message });
        ...
        public async Task<IActionResult> GetFrtCrAcByTransCode(int transportCode)
        {
            ...
            try
            {
                var result = await _purchaseBillPassEntry.GetFrtCrAcByTransCodeAsync(transportCode);
        ...
        public async Task<IActionResult> GetPurchaseDate(string vType, int vNo)
        {
            try
            {
                var result = await _purchaseBillPassEntry.GetPurchaseDate(vType, vNo);
                return Json(new { success = result.status, purchaseDate = result.data });
            }
            catch (Exception ex)
            {
                return Json(new { success = false, message = ex.Message });
        ...
        public async Task<IActionResult> GetPartyPurchaseAmount(int partyCode, string vType, int? vNo = null, decimal? currentAmount = null)
        {
            try
            {
                var result = await _purchaseBillPassEntry.GetPartyPurchaseAmount(partyCode, vType, vNo, currentAmount);
        ...
        public async Task<IActionResult> GetTDS206Apply(int partyCode)
        {
            try
            {
                var result = await _purchaseBillPassEntry.GetTDS206Apply(partyCode);
        ...
        public async Task<IActionResult> IsPostingExist(string vType)
        {
            try
            {
                var result = await _purchaseBillPassEntry.IsPostingExist(vType);
                return Json(new { success = result.status, isPostingExist = result.data });
            }
            catch (Exception ex)
            {
                return Json(new { success = false, message = ex.Message });
        ...
        public async Task<JsonResult> GetPLDocId(int vNo, int plNo)
        {
            try
            {
                var result = await _purchaseBillPassEntry.GetPLDocId(vNo, plNo);
                return Json(new { success = result.status, docId = result.data });
            }
            catch (Exception ex)
            {
                return Json(new { success = false, message = ex.Message });
        ...
        public async Task<JsonResult> GetDebitAcType(int code)
        {
            try
            {
                var result = await _purchaseBillPassEntry.GetDebitAcType(code);
                return Json(new { success = result.status, type = result.data });
            }
            catch (Exception ex)
            {
                return Json(new { success = false, message = ex.Message });
        ...
        public async Task<JsonResult> GetPOType(string poType, int poNo)
        {
            try
            {
                var result = await _purchaseBillPassEntry.GetPOType(poType, poNo);
                return Json(new { success = result.status, type = result.data });
            }
            catch (Exception ex)
            {
        ...
        public async Task<JsonResult> GetPurchaseOrSaleVoucherNo(string transportName, string grNo, string currentVoucher, string purchaseOrSale)
        {
            try
            {
                var result = await _purchaseBillPassEntry.GetPurchaseOrSaleVoucherNo(transportName, grNo, currentVoucher, purchaseOrSale);
        ...
        public async Task<JsonResult> CheckPaymentExists(string docType, int docNo)
        {
            try
            {
                var result = await _purchaseBillPassEntry.CheckPaymentExists(docType, docNo);
        ...
        public async Task<JsonResult> CheckDuplicateBill(int partyCode, string billNo, int currentVNo)
        {
            try
            {
                var result = await _purchaseBillPassEntry.CheckDuplicateBill(partyCode, billNo, currentVNo);
        ...
        public async Task<JsonResult> ValidateTaxType(int cityCode, decimal totalIGST, decimal totalCGST, decimal totalSGST)
        {
            try
            {
                var result = await _purchaseBillPassEntry.ValidateTaxType(cityCode, totalIGST, totalCGST, totalSGST);
        ...
        [HttpGet]
        public IActionResult GetAdvanceTdsList(string? billNo, int drCode)
        {
            billNo ??= "";
            var result = _purchaseBillPassEntry.GetAdvanceTdsList(billNo, drCode);

            return Json(new { success = result.status, message = result.message, data = result.data });
        }

        [HttpPost]
        public async Task<IActionResult> CalculateDrCrAmount([FromBody] CalculateDrCrRequest request)
        {
            try
            {
                var result = await _purchaseBillPassEntry.CalculateRowDrCrAmount(request);
        ...
        [HttpPost]
        public async Task<IActionResult> UpdatePassDetails()
        {
            try
            {
                var result = await _purchaseBillPassEntry.UpdatePassDetails();
        ...
        [HttpGet]
        public async Task<IActionResult> GetCrDrNoteAmtForReport(string vType, int vNo, string drOrCr)
        {
            var gv = _globalVariableService.GetGlobalVariables();
            try
            {
                var result = await _purchaseBillPassEntry.GetCrDrNoteAmtForReport(vType, vNo, drOrCr);
                return Json(new { success = result.status, data = result.data });
            }
            catch (Exception ex)
            {
        ...
        [HttpPost]
        public async Task<IActionResult> GenerateImportBill([FromBody] ImportBillRequest model)
        {
            try
            {
                bool result = await _purchaseBillPassEntry.GenerateImportBillAsync(model.VNo, model.VType, model.VDate, model.PlNo);

                return Json(new{success = result, message = "Import Bill generated successfully."});
            }
            catch (Exception ex)
            {
                return Json(new {success = false, message = ex.Message});
