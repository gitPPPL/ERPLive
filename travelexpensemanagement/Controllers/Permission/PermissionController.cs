...
namespace travelexpensemanagement.Controllers.Permission
{
    public class PermissionController : Controller
    {
        ...
        public IActionResult GetCurrentMenuPermission(string controllerName)
        {
            try
            {
                ...
                var permission = _moduleService.GetUserMenuPermissions().FirstOrDefault(x => x.MENU_CODE == menuId);
                if (permission == null)
                {
                    return Json(new
                    {
        ...
        [HttpGet]
        public IActionResult GetCurrentEntryPagePermission(string controllerName)
        {
            try
            {
                if (string.IsNullOrWhiteSpace(controllerName))
                ...
                using (SqlConnection conn = _dbConnection.GetErpConnection())
                {
                    ...
                    const string query = @"SELECT CODE
                           FROM MENU_MAST
                           WHERE LTRIM(RTRIM(WebFORM_MainNAME)) = @WebFORM_MainNAME";

                    using (SqlCommand cmd = new SqlCommand(query, conn))
                    {
                        cmd.Parameters.AddWithValue("@WebFORM_MainNAME", controllerName);
