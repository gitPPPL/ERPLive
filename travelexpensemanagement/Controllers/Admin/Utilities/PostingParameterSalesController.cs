...
{
    public class PostingParameterSalesController : Controller
    {
        ...
        public PostingParameterSalesController(DataBaseConnection dbcontext, travelexpensemanagement.DbHelper.DbHelper dbHelper, GlobalVariableService globalValue, ModuleService.ModuleService moduleService, Services.IMasterDataService masterDataService)
        {
            _dbHelper = dbHelper;
        ...
        {
            return View("~/Views/Admin/Utilities/PostingParameterSales/Index.cshtml");
        }
        ...
        public async Task<IActionResult> GetPostTypeList()
        {
            ...
            {
                var companyCd = _globalValue.GetGlobalVariables().PubCompCode;
                var dataList = await _dbHelper.GetJsonDataAsync($@"select distinct  SALE_GROUP as NAME from ITEM_GROUP where COMP_CODE={companyCd} and isnull(SALE_GROUP, '')<>''  order by  NAME ");
                return Json(new { status = true, data = dataList});
            }
            catch (Exception ex)
            {
        ...
        [HttpGet]
        public async Task<IActionResult> GetFormTypeList()
        {
            try
            {
                var companyCd = _globalValue.GetGlobalVariables().PubCompCode;
        ...
        public async Task<IActionResult> GetDocList()
        {
            try
            {
                ...
                where c.doctype not in ('salesinvoice','SalesReturn','JobworkIssue') and a.comp_code={companyCd} AND a.BRANCH_CODE= 1 and isnull(DOC_TYPE, '')='SALE' Order by a.Doc_Type
                ");
        ...
        [HttpGet]
        public async Task<IActionResult> GetPostingParameterForUpdate(string vType ,string postType, int formCode)
        {
            try
            {
                var companyCd = _globalValue.GetGlobalVariables().PubCompCode;
        ...
        [HttpPost]
        public async Task<IActionResult> SaveOrUpdatePostingParameterSaleEntry([FromBody] PostingParameter model)
        {
            if (model == null)
