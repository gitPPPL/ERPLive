...
{
    public class JobworkIssueChallanListController : Controller
    {
        ...
        public JobworkIssueChallanListController(DataBaseConnection dbcontext, travelexpensemanagement.DbHelper.DbHelper dbHelper, GlobalVariableService globalValue, ModuleService.ModuleService moduleService, Services.IMasterDataService masterDataService)
        {
            _dbHelper = dbHelper;
        ...
        public IActionResult Index()
        {
            ...
            return View("~/Views/Sales/Transaction/JobworkIssueChallanList/Index.cshtml", model);
        }

        [HttpGet]
        public async Task<IActionResult> GetJobworkIssueChallanList(string searchTerm = "", int pageNumber = 1, int pageSize = 10)
        {
            try
            {
                ...
                var fullList = await _dbHelper.GetJsonFromProcedureAsync("[dbo].[sp_SaleDataList]", parameter);
        ...
        [HttpDelete]
        public async Task<IActionResult> DeleteJobworkIssueChallanEntry(string docid)
        {
            try
            {
                ...
                using (var con = _dbcontext.GetErpConnection())
                {
                    ...
                    using (var transaction = con.BeginTransaction())
                    {
                        ...
                        "DELETE FROM sale1 WHERE COMP_CODE = @COMP_CODE AND YEAR_CODE = @YEAR_CODE AND BRANCH_CODE = @BRANCH_CODE AND V_TYPE = @V_TYPE AND V_NO = @V_NO",
                        "DELETE FROM sale2 WHERE COMP_CODE = @COMP_CODE AND YEAR_CODE = @YEAR_CODE AND BRANCH_CODE = @BRANCH_CODE AND V_TYPE = @V_TYPE AND V_NO = @V_NO"
                        };
        ...
        [HttpGet]
        public async Task<IActionResult> GetJobworkIssueChallanEntryDetails(string docid)
        {
            try
            {
                var usersession = _globalValue.GetGlobalVariables();
                ...
                var entryDetailList = await _dbHelper.GetJsonFromProcedureAsync("[dbo].[sp_SaleEntry]", parameter);
        ...
        public async Task<IActionResult> ExportAllDocs()
        {
            ...
            try
            {
                ...
                var dataList = await _dbHelper.GetJsonFromProcedureAsync("[dbo].[sp_SaleEntry]", parameter);
