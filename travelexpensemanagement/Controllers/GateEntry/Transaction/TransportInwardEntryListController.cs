...
{
    public class TransportInwardEntryListController : Controller
    {
        private readonly travelexpensemanagement.DbHelper.DbHelper _dbHelper;
        private readonly DataBaseConnection _dbcontext;
        private readonly GlobalVariableService _globalValue;
        private readonly travelexpensemanagement.ModuleService.ModuleService _moduleService;
        public TransportInwardEntryListController(DataBaseConnection dbcontext, travelexpensemanagement.DbHelper.DbHelper dbHelper, GlobalVariableService globalValue, ModuleService.ModuleService moduleService)
        {
            _dbHelper = dbHelper;
        ...
        public IActionResult Index()
        {
            ...
            return View("~/Views/GateEntry/Transaction/TransportInwardEntryList/Index.cshtml", model);
        }
        ...
        public async Task<IActionResult> GetTransportInwardList(string searchTerm = "", int pageNumber = 1, int pageSize = 10)
        {
            try
            {
                ...
                var parameter = new Dictionary<string, object>
                {
                    ...
                    {"@BRANCH_CODE", 1},
                    {"@Action", "TransportInwardEntryList" }
                };

                var fullList = await _dbHelper.GetJsonFromProcedureAsync("[dbo].[sp_GetTransportInwardEntry]", parameter);
        ...
        [HttpDelete]
        public async Task<IActionResult> DeleteTransportInwardEntry(string docid)
        {
            try
            {
                ...
                using (var con = _dbcontext.GetErpConnection())
                {
                    ...
                    using (var transaction = con.BeginTransaction())
                    {
                        ...
                        "DELETE FROM gate2 WHERE COMP_CODE = @COMP_CODE AND YEAR_CODE = @YEAR_CODE AND BRANCH_CODE = @BRANCH_CODE AND V_TYPE = @V_TYPE AND V_NO = @V_NO"
        ...
        [HttpGet]
        public async Task<IActionResult> GetTransportInwardEntryDetails(string docid)
        {
            try
            {
                var usersession = _globalValue.GetGlobalVariables();
