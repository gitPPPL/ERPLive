...
{
    public class EmployeeGateOutListController : Controller
    {
        private readonly travelexpensemanagement.DbHelper.DbHelper _dbHelper;
        private readonly DataBaseConnection _dbcontext;
        private readonly GlobalVariableService _globalValue;
        private readonly travelexpensemanagement.ModuleService.ModuleService _moduleService;
        public EmployeeGateOutListController(DataBaseConnection dbcontext, travelexpensemanagement.DbHelper.DbHelper dbHelper, GlobalVariableService globalValue, ModuleService.ModuleService moduleService)
        {
            _dbHelper = dbHelper;
        ...
        public IActionResult Index()
        {
            ...
            return View("~/Views/Payroll/Transaction/EmployeeGateOutList/Index.cshtml", model);
        }

        [HttpGet]
        public async Task<IActionResult> GetGateInOutList(string searchTerm = "", int pageNumber = 1, int pageSize = 10)
        {
            try
            {
                ...
                var parameter = new Dictionary<string, object>
                {
                    ...
                    {"@BRANCH_CODE", 1},
                    {"@V_TYPE", "ATAD" },
                    {"@Action", "GateInOutEntryList" }
                };

                var fullList = await _dbHelper.GetJsonFromProcedureAsync("[dbo].[sp_GateInOutEntry]", parameter);
        ...
        [HttpDelete]
        public async Task<IActionResult> DeleteGateInOutEntry(string docid)
        {
            try
            {
                ...
                using (var con = _dbcontext.GetErpConnection())
                {
                    try
                    {
                        string query = "DELETE FROM PAY_INOUT WHERE COMP_CODE = @COMP_CODE AND YEAR_CODE = @YEAR_CODE AND BRANCH_CODE = @BRANCH_CODE AND V_TYPE = @V_TYPE AND V_NO = @V_NO";
        ...
        [HttpGet]
        public async Task<IActionResult> GetGateInOutEntryDetails(string docid)
        {
            try
            {
                var usersession = _globalValue.GetGlobalVariables();
                ...
                var entryDetailList = await _dbHelper.GetJsonFromProcedureAsync("[dbo].[sp_GateInOutEntry]", parameter);
