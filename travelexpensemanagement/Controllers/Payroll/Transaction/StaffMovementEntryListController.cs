...
{
    public class StaffMovementEntryListController : Controller
    {

        private readonly DataBaseConnection _dbConnection;
        private readonly GlobalVariableService _globalVariableService;
        public StaffMovementEntryListController(DataBaseConnection dbConnection, GlobalVariableService globalVariableService,
        ...
        {
            return View("~/Views/Payroll/Transaction/StaffMovementEntryList/Index.cshtml");
        }
        ...
        public IActionResult Getlist(string searchTerm = "", int pageNumber = 1, int pageSize = 10)
        {
            ...
            var StaffMovementEntry_Model = new List<StaffMovementEntry_Model>();
            int totalCount = 0;
            ...
            try
            {
                ...
                using (var cmd = new SqlCommand("sp_StaffMovementEntry", conn))
                {
                    ...
                    using (var reader = cmd.ExecuteReader())
                    {
                        while (reader.Read())
                        {
                            StaffMovementEntry_Model.Add(new StaffMovementEntry_Model
                            {
            ...
            return Json(new { success = true, lists = StaffMovementEntry_Model, totalCount });
        }
        ...
        public IActionResult GetdataByCode(string code)
        {
            ...
            StaffMovementEntry_Model StaffMovementEntry_Model = null;

            try
            {
                using (SqlConnection con = _dbConnection.GetErpConnection())
                {
                    using (SqlCommand cmd = new SqlCommand("sp_StaffMovementEntry", con))
                    {
                        cmd.CommandType = CommandType.StoredProcedure;
                        ...
                        using (SqlDataReader rdr = cmd.ExecuteReader())
                        {
                            if (rdr.Read())
                            {
                                StaffMovementEntry_Model = new StaffMovementEntry_Model
                                {
                ...
                return Json(new { success = true, data = StaffMovementEntry_Model });
            }
            catch (Exception ex)
            {
        ...
        public JsonResult Delete(string code)
        {
            ...
            try
            {
                ...
                return Json(new { success = true, message = "Staff Movement Entry deleted successfully." });
            }
            catch (Exception ex)
            {
                return Json(new { success = false, message = "Error deleting Staff Movement Entry .", error = ex.Message });
            }
        }
