...
{
    public class SalaryLevelMasterController : Controller
    {
        ...
        public SalaryLevelMasterController(DataBaseConnection dbcontext, travelexpensemanagement.DbHelper.DbHelper dbHelper, GlobalVariableService globalValue, ModuleService.ModuleService moduleService)
        {
            _dbHelper = dbHelper;
        ...
        public IActionResult Index()
        {
            ...
            return View("~/Views/Payroll/Master/SalaryLevelMaster/Index.cshtml", model);
        }

        [HttpPost]
        public IActionResult SaveMaster([FromBody] SalaryLevelModel data)
        {
            if (data == null)
            {
        ...
        [HttpPost]
        private string Submitbtn(SalaryLevelModel data, string action)
        {
            try
            {
                ...
                using (SqlConnection conn = _dbConnection.GetErpConnection())
                {
                    ...
                    using (SqlCommand cmd = new SqlCommand("sp_SalaryLevel", conn))
                    {
                        ...
                        cmd.Parameters.AddWithValue("@BASIC", data.BASIC);
                        cmd.Parameters.AddWithValue("@HRA", data.HRA);
                        cmd.Parameters.AddWithValue("@CONV", data.CONV);
                        cmd.Parameters.AddWithValue("@OTHERS", data.OTHERS);
                        cmd.Parameters.AddWithValue("@TOT_AMT", data.TOT_AMT);
                        cmd.Parameters.AddWithValue("@GW_AMT", data.GW_AMT);
        ...
        public JsonResult Getcode()
        {
            ...
            {
                string query = @"
                 select max(code) + 1  from SALARY_LEVEL   WHERE COMP_CODE = @CompCode ; ";
        ...
        public JsonResult SearchNames(string term)
        {
            ...
            try
            {
                ...
                {
                    string query = @"
                  select NAME from SALARY_LEVEL
                    WHERE NAME LIKE @SearchText and  COMP_CODE =@COMP_CODE 
                    ORDER BY NAME ASC";
