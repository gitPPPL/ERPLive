...
{
    public class DepartmentCategoryMasterListController : Controller
    {
        private readonly travelexpensemanagement.DbHelper.DbHelper _dbHelper;
        private readonly DataBaseConnection _dbcontext;
        private readonly GlobalVariableService _globalValue;

        public DepartmentCategoryMasterListController(DataBaseConnection dbcontext, travelexpensemanagement.DbHelper.DbHelper dbHelper, GlobalVariableService globalValue)
        {
            _dbHelper = dbHelper;
        ...
        {
            return View("~/Views/Payroll/Master/DepartmentCategoryMasterList/Index.cshtml");
        }

        [HttpGet]
        public async Task<IActionResult> GetDepCategoryData(string searchTerm = "", int pageNumber = 1, int pageSize = 10)
        {
            try
            {
                var UsersessionDt = _globalValue.GetGlobalVariables();
                string strqry = $@"
               select distinct CODE,NAME,SHORTNAME,ACTIVE from CAT_MAST
        ...
        [HttpDelete]
        public async Task<IActionResult> DelDepCategoryMastDt(int Code)
        {
            try
            {
                int x;
