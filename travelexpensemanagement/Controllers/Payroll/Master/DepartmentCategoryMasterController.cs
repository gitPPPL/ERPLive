...
{
    public class DepartmentCategoryMasterController : Controller
    {
        private readonly travelexpensemanagement.DbHelper.DbHelper _dbHelper;
        private readonly DataBaseConnection _dbcontext;
        private readonly GlobalVariableService _globalValue;
        int x;
        public DepartmentCategoryMasterController(DataBaseConnection dbcontext, travelexpensemanagement.DbHelper.DbHelper dbHelper, GlobalVariableService globalValue)
        {
            _dbHelper = dbHelper;
        ...
        {
            return View("~/Views/Payroll/Master/DepartmentCategoryMaster/Index.cshtml");
        }
        public class departmentCatModel
        {
            ...
            try
            {
                ...
                using (var con = _dbcontext.GetErpConnection())
                {
                    using (SqlCommand cmd = new SqlCommand())
                    {
                        cmd.Connection = con;
                        cmd.CommandText = @"
                        SELECT CASE WHEN EXISTS (SELECT 1 FROM CAT_MAST WHERE UPPER(ISNULL(NAME, '')) = UPPER(@Inputdata) AND COMP_CODE = @CompCode) 
                        THEN 1 ELSE 0 END";
        ...
        [HttpPost]
        public async Task<IActionResult> SaveDepCategoryMast([FromBody] departmentCatModel model)
        {
            try
            {
                if (model == null)
                ...
                using (var con = _dbcontext.GetErpConnection())
                {
                    ...
                    using (SqlCommand cmd = new SqlCommand("[dbo].[sp_DepartmentCategoryMast_AED]", con))
                    {
        ...
        [HttpGet]
        public async Task<IActionResult> GetDepCategoryDetailsById(string id)
        {
            try
            {
                string strqry = $@"
                select CODE,NAME,SHORTNAME,ACTIVE from CAT_MAST
        ...
        [HttpPost]
        public async Task<IActionResult> UpdateDepCategoryMast([FromBody] departmentCatModel model)
        {
            try
            {
                if (model == null)
