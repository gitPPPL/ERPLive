...
{
    public class SalaryUnpaidListController : Controller
    {

        private readonly DataBaseConnection _dbConnection;
        private readonly GlobalVariableService _globalVariableService;
        private int? userLevel;
        public SalaryUnpaidListController(DataBaseConnection dbConnection, GlobalVariableService globalVariableService,
    ...
    ModuleService.ModuleService moduleService)
        ...
        {
            return View("~/Views/Payroll/MonthlyTransaction/SalaryUnpaidList/Index.cshtml");
        }

        public IActionResult GetList(string searchTerm = "", int pageNumber = 1, int pageSize = 10)
        {
            var getdata = _globalVariableService.GetGlobalVariables();
            var GatePassEntry = new List<MonthyTransaction_Model>();
            int totalCount = 0;
            ...
            try
            {
                ...
                using (var cmd = new SqlCommand("sp_SalaryUnpaidENTRY", conn))
                {
                    ...
                    using (var reader = cmd.ExecuteReader())
                    {
                        while (reader.Read())
                        {
                            GatePassEntry.Add(new MonthyTransaction_Model
                            {
        ...
        public IActionResult GetDataByCode(string code)
        {
            ...
            try
            {
                using (SqlConnection con = _dbConnection.GetErpConnection())
                {
                    ...
                    // Header
                    using (SqlCommand cmd = new SqlCommand("sp_SalaryUnpaidENTRY", con))
                    {
                        cmd.CommandType = CommandType.StoredProcedure;
                    ...
                    // Details
                    using (SqlCommand cmd2 = new SqlCommand("sp_SalaryUnpaidENTRY", con))
                    {
        ...
        public JsonResult Delete(string code)
        {
            ...
            try
            {
                ...
                return Json(new { success = true, message = " Salary Unpaid Entry deleted successfully." });
            }
            catch (Exception ex)
            {
                return Json(new { success = false, message = "Error deleting  Salary Unpaid Entry  .", error = ex.Message });
            }
        }
