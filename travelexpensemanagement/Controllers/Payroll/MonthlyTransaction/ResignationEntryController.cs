...
{
    public class ResignationEntryController : Controller
    {
        ...
        public ResignationEntryController(DataBaseConnection dbConnection, GlobalVariableService globalVariableService,
        ...
        {
            return View("~/Views/Payroll/MonthlyTransaction/ResignationEntry/Index.cshtml");
        }
        public JsonResult GetVNo()
        {
            ...
            try
            {
                ...
                using (SqlConnection con = _dbConnection.GetErpConnection())
                {
                    ...
                    string lastV_NO_Query = "select max(V_no) from PAY_RESIGN where V_TYPE='RESN' and COMP_CODE= @CompCode and BRANCH_CODE= 1 and YEAR_CODE= @YearCode  ";
        ...
        public JsonResult DDlEmployee()
        {
            ...
            using (SqlConnection con = _dbConnection.GetErpConnection())
            {
                string query = "select CODE,NAME from EMP_MAST where RESIGN_DATE IS NOT NULL  and ACTIVE = 1 and COMP_CODE = " + getdata.PubCompCode +"    order by NAME";
        ...
        [HttpPost]
        public IActionResult SaveTempMaster([FromBody] ResignationEntry_Model data)
        {
            if (data == null)
            {
        ...
        [HttpPost]
        private string Submitbtn(ResignationEntry_Model data, string action)
        {
            try
            {
                ...
                using (SqlConnection conn = _dbConnection.GetErpConnection())
                {
                    ...
                    using (SqlCommand cmd = new SqlCommand("sp_PAY_RESIGN", conn))
                    {
                        cmd.CommandType = CommandType.StoredProcedure;
                        cmd.Parameters.AddWithValue("@Action", action);
                        cmd.Parameters.AddWithValue("@COMP_CODE", globalVar.PubCompCode);
                        cmd.Parameters.AddWithValue("@DOC_ID", "RESN" + data.V_NO);
                        cmd.Parameters.AddWithValue("@YEAR_CODE", globalVar.PubFYearCode);
                        cmd.Parameters.AddWithValue("@BRANCH_CODE", 1);
                        cmd.Parameters.AddWithValue("@V_TYPE", "RESN");
                        ...
                        cmd.Parameters.AddWithValue("@RESIGN_DATE", data.RESIGN_DATE);
                        cmd.Parameters.AddWithValue("@RELIEVING_DATE", data.RELIEVING_DATE);
                        cmd.Parameters.AddWithValue("@LAST_WORK_DATE", data.LAST_WORK_DATE);
                        cmd.Parameters.AddWithValue("@RESIGN_REASON", data.RESIGN_REASON);
                        cmd.Parameters.AddWithValue("@REMARKS", data.REMARKS);
                        cmd.Parameters.AddWithValue("@ATTACH1", data.ATTACH1);
                        cmd.Parameters.AddWithValue("@ATTACH2", data.ATTACH2);
                        cmd.Parameters.AddWithValue("@ATTACH3", data.ATTACH3);
