...
{
    public class SalaryHoldListController : Controller
    {
        ...
        public SalaryHoldListController(DataBaseConnection dbConnection, GlobalVariableService globalVariableService,
    ...
    ModuleService.ModuleService moduleService)
        ...
        {
            return View("~/Views/Payroll/MonthlyTransaction/SalaryHoldList/Index.cshtml");
        }

        public IActionResult GetList(string searchTerm = "", int pageNumber = 1, int pageSize = 10)
        {
            var getdata = _globalVariableService.GetGlobalVariables();
            var GatePassEntry = new List<SalaryHoldEntry_Model>();
            int totalCount = 0;

            try
            {
                using (var conn = _dbConnection.GetErpConnection())
                using (var cmd = new SqlCommand("sp_SalaryHold", conn))
                {
                    ...
                    using (var reader = cmd.ExecuteReader())
                    {
                        while (reader.Read())
                        {
                            GatePassEntry.Add(new SalaryHoldEntry_Model
                            {
        ...
        public IActionResult GetDataByCode(string code)
        {
            ...
            try
            {
                using (SqlConnection con = _dbConnection.GetErpConnection())
                {
                    ...
                    // Header
                    using (SqlCommand cmd = new SqlCommand("sp_SalaryHold", con))
                    {
                        cmd.CommandType = CommandType.StoredProcedure;
                    ...
                    // Details
                    using (SqlCommand cmd2 = new SqlCommand("sp_SalaryHold", con))
                    {
        ...
        public JsonResult Delete(string code)
        {
            ...
            try
            {
                ...
                return Json(new { success = true, message = " Salary Hold Entry deleted successfully." });
            }
            catch (Exception ex)
            {
                return Json(new { success = false, message = "Error deleting  Salary Hold Entry  .", error = ex.Message });
            }
        }
