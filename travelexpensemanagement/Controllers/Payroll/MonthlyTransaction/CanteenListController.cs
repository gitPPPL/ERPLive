...
{
       public class CanteenListController : Controller
    {

        private readonly DataBaseConnection _dbConnection;
        private readonly GlobalVariableService _globalVariableService;
        private int? userLevel;
        public CanteenListController(DataBaseConnection dbConnection, GlobalVariableService globalVariableService,
    ...
    ModuleService.ModuleService moduleService)
        ...
        {
            return View("~/Views/Payroll/MonthlyTransaction/CanteenList/Index.cshtml");
        }
        ...
        public IActionResult GetList(string searchTerm = "", int pageNumber = 1, int pageSize = 10)
        {
            ...
            var GatePassEntry = new List<Canteen_Model>();
            int totalCount = 0;
            ...
            try
            {
                ...
                using (var cmd = new SqlCommand("sp_Canteen", conn))
                {
                    ...
                    using (var reader = cmd.ExecuteReader())
                    {
                        while (reader.Read())
                        {
                            GatePassEntry.Add(new Canteen_Model
                            {
        ...
        public IActionResult GetDataByCode(string code)
        {
            ...
            try
            {
                using (SqlConnection con = _dbConnection.GetErpConnection())
                {
                    ...
                    // Header
                    using (SqlCommand cmd = new SqlCommand("sp_Canteen", con))
                    {
                        cmd.CommandType = CommandType.StoredProcedure;
                    ...
                    // Details
                    using (SqlCommand cmd2 = new SqlCommand("sp_Canteen", con))
                    {
        ...
        public JsonResult Delete(string code)
        {
            ...
            try
            {
                ...
                return Json(new { success = true, message = "Canteen Entry deleted successfully." });
            }
            catch (Exception ex)
            {
                return Json(new { success = false, message = "Error deleting  Canteen Entry  .", error = ex.Message });
            }
        }
