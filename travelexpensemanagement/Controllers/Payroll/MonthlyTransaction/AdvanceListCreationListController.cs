...
{
    public class AdvanceListCreationListController : Controller
    {
        ...
        public AdvanceListCreationListController(DataBaseConnection dbcontext, travelexpensemanagement.DbHelper.DbHelper dbHelper, GlobalVariableService globalValue, ModuleService.ModuleService moduleService)
        {
            _dbHelper = dbHelper;
        ...
        {
            ViewBag.CurrentMenu = "Advance List Creation";
            ...
            return View("~/Views/Payroll/MonthlyTransaction/AdvanceListCreationList/Index.cshtml", model);
        }

        [HttpGet]
        public async Task<IActionResult> GetAdvanceListCreationList(string searchTerm = "", int pageNumber = 1, int pageSize = 10)
        {
            try
            {
                ...
                var parameter = new Dictionary<string, object>
                {
                    ...
                    {"@BRANCH_CODE", 1 },                  
                    {"@Action", "AdvanceListCreationList" }
                };

                var fullList = await _dbHelper.GetJsonFromProcedureAsync("[dbo].[sp_AdvanceListCreation]", parameter);
        ...
        [HttpDelete]
        public async Task<IActionResult> DeleteAdvanceListCreationEntry(string docid)
        {
            try
            {
        ...
        [HttpGet]
        public async Task<IActionResult> GetAdvanceListCreationEntryDetails(string docid)
        {
            try
            {
                var usersession = _globalValue.GetGlobalVariables();
                ...
                var entryDetailList = await _dbHelper.GetJsonFromProcedureAsync("[dbo].[sp_AdvanceListCreation]", parameter);
        ...
        public async Task<IActionResult> ExportAllDocs()
        {
            try
            {
                ...
                var dataList = await _dbHelper.GetJsonFromProcedureAsync("[dbo].[sp_AdvanceListCreation]", parameter);
