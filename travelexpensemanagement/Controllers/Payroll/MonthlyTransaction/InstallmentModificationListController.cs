...
{
    public class InstallmentModificationListController : Controller
    {
    ...
    ModuleService.ModuleService moduleService)
        ...
        {
            return View("~/Views/Payroll/MonthlyTransaction/InstallmentModificationList/Index.cshtml");
        }
        ...
        public IActionResult GetList(string searchTerm = "", int pageNumber = 1, int pageSize = 10)
        {
            ...
            var InstallmentModificationEntry_model = new List<InstallmentModificationEntry_model>();
            int totalCount = 0;

            try
            {
                using (var conn = _dbConnection.GetErpConnection())
                using (var cmd = new SqlCommand("sp_InstallmentModificationEntry", conn))
                {
                    ...
                    using (var reader = cmd.ExecuteReader())
                    {
                        while (reader.Read())
                        {
                            InstallmentModificationEntry_model.Add(new InstallmentModificationEntry_model
                            {
            ...
            return Json(new { success = true, lists = InstallmentModificationEntry_model, totalCount });
        }
        ...
        public IActionResult GetDataByCode(string code)
        {
            ...
            try
            {
                using (SqlConnection con = _dbConnection.GetErpConnection())
                {
                    ...
                    // Header
                    using (SqlCommand cmd = new SqlCommand("sp_InstallmentModificationEntry", con))
                    {
                        cmd.CommandType = CommandType.StoredProcedure;
                    ...
                    // Details
                    using (SqlCommand cmd2 = new SqlCommand("sp_InstallmentModificationEntry", con))
                    {
        ...
        public JsonResult Delete(string code)
        {
            ...
            try
            {
                ...
                return Json(new { success = true, message = "Installment Modification Entry  deleted successfully." });
            }
            catch (Exception ex)
            {
                return Json(new { success = false, message = "Error deleting Installment Modification Entry .", error = ex.Message });
            }
        }
