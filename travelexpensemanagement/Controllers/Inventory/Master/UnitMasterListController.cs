...
{
    public class UnitMasterListController : Controller
    {
        ...
        public UnitMasterListController(DataBaseConnection dbConnection, GlobalVariableService globalVariableService,
    DropdownService dropdownService, DbHelper dbHelper, ModuleService.ModuleService moduleService)
        ...
        {
            return View("~/Views/Inventory/Master/UnitMasterList/Index.cshtml");
        }

        [HttpGet]
        public IActionResult GetAllItemUnits(string searchTerm = "", int pageNumber = 1, int pageSize = 10)
        {
            var compCode = _globalVariableService.GetGlobalVariables().PubCompCode;
            var colorGroups = new List<ITEMUNIT_MAST>();
            int totalCount = 0;
            ...
            try
            {
                ...
                {
                    using (SqlCommand cmd = new SqlCommand("sp_ITEMUNIT_MAST", conn))
                    {
                        ...
                        using (SqlDataReader reader = cmd.ExecuteReader())
                        {
                            while (reader.Read())
                            {
                                colorGroups.Add(new ITEMUNIT_MAST
                                {
        ...
        [HttpGet]
        public IActionResult GetItemUnitByCode(int code)
        {
            var globalVar = _globalVariableService.GetGlobalVariables();
            ITEMUNIT_MAST group = null;
