...
{
    public class ColorGroupMasterListController : Controller
    {
        ...
        public ColorGroupMasterListController(DataBaseConnection dbConnection, GlobalVariableService globalVariableService,
    DropdownService dropdownService, DbHelper dbHelper, ModuleService.ModuleService moduleService)
        ...
        {
            return View("~/Views/Inventory/Master/ColorGroupMasterList/Index.cshtml");
        }

        [HttpGet]
        public IActionResult GetAllColorGroups(string searchTerm = "", int pageNumber = 1, int pageSize = 10)
        {
            var compCode = _globalVariableService.GetGlobalVariables().PubCompCode;
            var colorGroups = new List<COLORGROUP_MAST>();
            int totalCount = 0;
            ...
            try
            {
                ...
                {
                    using (SqlCommand cmd = new SqlCommand("sp_COLORGROUP_MAST", conn))
                    {
                        ...
                        using (SqlDataReader reader = cmd.ExecuteReader())
                        {
                            while (reader.Read())
                            {
                                colorGroups.Add(new COLORGROUP_MAST
                                {
                                    ...
                                    COLOR_TYPE = reader["COLOR_TYPE"]?.ToString(),
        ...
        [HttpGet]
        public IActionResult GetColorGroupByCode(int code)
        {
            var globalVar = _globalVariableService.GetGlobalVariables();
            COLORGROUP_MAST group = null;
