        ...
        public async Task<IActionResult> ExportToExcel(string searchTerm = null)
        {
            ...
            using (var conn = _dbConnection.GetErpConnection())
            {
                ...
                using (SqlCommand cmd = new SqlCommand("sp_InventoryTransferRequest", conn))
                {
                    ...
                    {
                        var ws = workbook.Worksheets.Add("InventoryTransferRequest");
                        ...
                        using (var stream = new MemoryStream())
                        {
                            ...
                            return File(
                                stream.ToArray(),
                                "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",  "InventoryTransferRequest.xlsx"
                            );
                        }
                    }
                }
