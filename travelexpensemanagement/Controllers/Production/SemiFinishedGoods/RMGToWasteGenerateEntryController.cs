...
using travelexpensemanagement.Models.Production.SemiFinishedGoods.RMGToWasteGenerateEntry;

namespace travelexpensemanagement.Controllers.Production.SemiFinishedGoods
{
    public class RMGToWasteGenerateEntryController : Controller
    {
        ...
        public RMGToWasteGenerateEntryController(DataBaseConnection dbConnection, GlobalVariableService globalVariableService,
     ...
     ModuleService.ModuleService moduleService)
        ...
        {
            return View("~/Views/Production/SemiFinishedGoods/RMGToWasteGenerateEntry/Index.cshtml");
        }
        ...
        public IActionResult DOCTYPE()
        {
            ...
            string query = "Select Code,Name from DOCTYPE_MAST where CODE in ('RAIT','RAIV')";
        ...
        [HttpGet]
        public IActionResult ItemMasterRAIT()
        {
            var getData = _globalVariableService.GetGlobalVariables();
            string compCode = getData.PubCompCode;
            string query = @"SELECT a.CODE AS value, a.NAME + '|' + ISNULL(b.NAME,'') AS text, a.NAME AS [item name], a.CODE AS icode, b.NAME AS unit, b.CODE AS ucode,
                            a.SHORTNAME AS [Short Name] FROM ITEM_MAST a LEFT JOIN ITEMUNIT_MAST b ON a.UNIT_CODE = b.CODE AND b.COMP_CODE = " + compCode + @"
                            INNER JOIN ITEM_MGROUP c ON a.MGROUP_CODE = c.CODE AND c.COMP_CODE = " + compCode + @" AND c.REPORT_TYPE IN ('Raw')WHERE a.COMP_CODE = " + compCode + @" 
                            AND a.Active = 1 GROUP BY a.CODE, a.NAME, b.NAME, b.CODE, a.SHORTNAME ORDER BY a.NAME";
            var docType = _dropdownService.GetDropdownList(query);
            return Json(new { success = true, data = docType });
        }

        [HttpGet]
        public IActionResult ItemMasterRAIV()
        {
            var getData = _globalVariableService.GetGlobalVariables();
            string compCode = getData.PubCompCode;
            string query = @"SELECT a.CODE AS value, a.NAME + '|' + ISNULL(b.NAME,'') AS text, a.NAME AS [item name], a.CODE AS icode, b.NAME AS unit,
                            b.CODE AS ucode, a.SHORTNAME AS [Short Name] FROM ITEM_MAST a LEFT JOIN ITEMUNIT_MAST b ON a.UNIT_CODE = b.CODE 
                            AND b.COMP_CODE = " + compCode + @" INNER JOIN ITEM_MGROUP c ON a.MGROUP_CODE = c.CODE AND c.COMP_CODE = " + compCode + @" AND c.REPORT_TYPE IN ('Waste')
                            WHERE a.COMP_CODE = " + compCode + @" AND a.Active = 1 GROUP BY a.CODE, a.NAME, b.NAME, b.CODE, a.SHORTNAME ORDER BY a.NAME";
            var docType = _dropdownService.GetDropdownList(query);
            return Json(new { success = true, data = docType });
        }
        ...
        [HttpPost]
        public IActionResult SaveAndUpdateData([FromBody] RMGToWasteGenerateEntry model)
        {
            if (model == null)
            {
            ...
            try
            {
                ...
                using (SqlConnection con = _dbConnection.GetErpConnection())
                {
                    ...
                    try
                    {
                        SqlCommand cmd = new SqlCommand("sp_RMGToWaste_GenerateEntry", con, trans);
                        ...
                        {
                            SqlCommand cmdItem = new SqlCommand("sp_RMGToWaste_GenerateEntry", con, trans);
        ...
        {
            var globalVariable = _globalVariableService.GetGlobalVariables();
            var model = new RMGToWasteGenerateEntry();
            ...
            try
            {
                using (SqlConnection con = _dbConnection.GetErpConnection())
                {
                    ...
                    if (reader.Read())
                    {
                        model = new RMGToWasteGenerateEntry
                        {
