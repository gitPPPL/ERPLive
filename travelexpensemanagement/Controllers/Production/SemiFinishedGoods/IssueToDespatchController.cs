...
using travelexpensemanagement.Models.Production.SemiFinishedGoods.IssueToDespatch;

namespace travelexpensemanagement.Controllers.Production.SemiFinishedGoods
{
    public class IssueToDespatchController : Controller
    {
        ...
        public IssueToDespatchController(DataBaseConnection dbConnection, GlobalVariableService globalVariableService,
     ...
     ModuleService.ModuleService moduleService)
        ...
        {
            return View("~/Views/Production/SemiFinishedGoods/IssueToDespatch/Index.cshtml");
        }
        ...
        public IActionResult DOCTYPE()
        {
            ...
            string query = "SELECT Code, Name FROM DOCTYPE_MAST WHERE CODE IN ('RAID');";
        ...
        public IActionResult ItemMaster()
        {
            ...
            string query = @" SELECT a.CODE AS value, a.name + '|' + ISNULL(b.NAME,'') AS text,a.CODE AS icode,a.name AS [item name], b.NAME AS unit, 
                               b.CODE AS ucode FROM item_mast a LEFT JOIN ITEMUNIT_MAST b ON a.UNIT_CODE = b.CODE AND b.comp_code = " + compCode + @" INNER JOIN ITEM_MGROUP c 
                               ON a.mGROUP_CODE = c.CODE AND c.COMP_CODE = " + compCode + @" AND c.REPORT_TYPE NOT IN ('Store', 'Fuel', 'Finish') WHERE a.comp_code = " + compCode + @"  
                               AND a.Active = 1 GROUP BY a.name, a.CODE, b.NAME, b.CODE ORDER BY a.name";
            var docType = _dropdownService.GetDropdownList(query);
            return Json(new { success = true, data = docType });
        }
        ...
        [HttpPost]
        public IActionResult SaveData([FromBody] IssueToDespatch model)
        {
            if (model == null)
            {
            ...
            try
            {
                ...
                using (SqlConnection con = _dbConnection.GetErpConnection())
                {
                    ...
                    try
                    {
                        SqlCommand cmd = new SqlCommand("sp_IssueTo_Despatch", con, trans);
                        ...
                        {
                            SqlCommand cmdItem = new SqlCommand("sp_IssueTo_Despatch", con, trans);
        ...
        public IActionResult loadDataOnEdit(string docId)
        {
            ...
            try
            {
                using (SqlConnection con = _dbConnection.GetErpConnection())
                {
                    SqlCommand cmd = new SqlCommand("sp_IssueTo_Despatch", con);
                    cmd.CommandType = CommandType.StoredProcedure;
