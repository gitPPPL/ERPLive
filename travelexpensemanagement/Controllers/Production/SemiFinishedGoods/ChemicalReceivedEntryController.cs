...
using travelexpensemanagement.Models.Production.SemiFinishedGoods.ChemicalReceivedEntry;

namespace travelexpensemanagement.Controllers.Production.SemiFinishedGoods
{
    public class ChemicalReceivedEntryController : Controller
    {
        ...
        public ChemicalReceivedEntryController(DataBaseConnection dbConnection, GlobalVariableService globalVariableService,
     ...
     ModuleService.ModuleService moduleService)
        ...
        {
            return View("~/Views/Production/SemiFinishedGoods/ChemicalReceivedEntry/Index.cshtml");
        }
        ...
        public IActionResult DOCTYPE()
        {
            ...
            string query = "Select Code,Name from DOCTYPE_MAST where CODE in ('CMRC')";
        ...
        public IActionResult ItemMaster()
        {
            ...
            string query = @"SELECT a.CODE AS value, a.NAME + ' | ' + ISNULL(b.NAME,'') AS text, b.NAME AS unit, b.CODE AS ucode, a.SHORTNAME AS [Short Name] 
                  FROM ITEM_MAST a LEFT JOIN (SELECT DISTINCT CODE, NAME, COMP_CODE FROM ITEMUNIT_MAST WHERE COMP_CODE = " + compCode + @") b 
                  ON a.UNIT_CODE = b.CODE INNER JOIN (SELECT DISTINCT CODE, COMP_CODE FROM ITEM_MGROUP WHERE COMP_CODE = " + compCode + @") c 
                  ON a.MGROUP_CODE = c.CODE WHERE a.COMP_CODE = " + compCode + @" AND a.ACTIVE = 1 ORDER BY a.NAME";
            var docType = _dropdownService.GetDropdownList(query);
            return Json(new { success = true, data = docType });
        }
        ...
        [HttpPost]
        public IActionResult SaveAndUpdateData([FromBody] ChemicalReceivedEntry model)
        {
            if (model == null)
            {
            ...
            try
            {
                ...
                using (SqlConnection con = _dbConnection.GetErpConnection())
                {
                    ...
                    try
                    {
                        SqlCommand cmd = new SqlCommand("sp_Chemcial_Received_Entry", con, trans);
                        ...
                        {
                            SqlCommand cmdItem = new SqlCommand("sp_Chemcial_Received_Entry", con, trans);
        ...
        {
            var globalVariable = _globalVariableService.GetGlobalVariables();
            var model = new ChemicalReceivedEntry();
            ...
            try
            {
                using (SqlConnection con = _dbConnection.GetErpConnection())
                {
                    SqlCommand cmd = new SqlCommand("sp_Chemcial_Received_Entry", con);
                    cmd.CommandType = CommandType.StoredProcedure;
                    ...
                    if (reader.Read())
                    {
                        model = new ChemicalReceivedEntry
                        {
