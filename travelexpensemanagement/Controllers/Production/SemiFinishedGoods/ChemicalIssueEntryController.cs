...
using travelexpensemanagement.Models.Production.SemiFinishedGoods.ChemicalIssueEntry;

namespace travelexpensemanagement.Controllers.Production.SemiFinishedGoods
{
    public class ChemicalIssueEntryController : Controller
    {
        ...
        public ChemicalIssueEntryController(DataBaseConnection dbConnection, GlobalVariableService globalVariableService,
     ...
     ModuleService.ModuleService moduleService)
        ...
        {
            return View("~/Views/Production/SemiFinishedGoods/ChemicalIssueEntry/Index.cshtml");
        }
        ...
        public IActionResult DOCTYPE()
        {
            ...
            string query = "Select Code,Name from DOCTYPE_MAST where CODE in ('CMIS')";
        ...
        [HttpPost]
        public IActionResult SaveAndUpdateData([FromBody] ChemicalIssueEntry model)
        {
            if (model == null)
            {
            ...
            try
            {
                ...
                using (SqlConnection con = _dbConnection.GetErpConnection())
                {
                    ...
                    try
                    {
                        SqlCommand cmd = new SqlCommand("sp_Chemical_Issue_Entry", con, trans);
                        ...
                        {
                            SqlCommand cmdItem = new SqlCommand("sp_Chemical_Issue_Entry", con, trans);
        ...
        {
            var globalVariable = _globalVariableService.GetGlobalVariables();
            var model = new ChemicalIssueEntry();
            ...
            try
            {
                using (SqlConnection con = _dbConnection.GetErpConnection())
                {
                    SqlCommand cmd = new SqlCommand("sp_Chemical_Issue_Entry", con);
                    cmd.CommandType = CommandType.StoredProcedure;
