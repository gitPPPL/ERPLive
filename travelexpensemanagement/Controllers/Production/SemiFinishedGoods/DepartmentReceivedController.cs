...
using travelexpensemanagement.Models.Production.SemiFinishedGoods.DepartmentReceived;

namespace travelexpensemanagement.Controllers.Production.SemiFinishedGoods
{
    public class DepartmentReceivedController : Controller
    {
        ...
        public DepartmentReceivedController(DataBaseConnection dbConnection, GlobalVariableService globalVariableService,
     ...
     ModuleService.ModuleService moduleService)
        ...
        {
            return View("~/Views/Production/SemiFinishedGoods/DepartmentReceived/Index.cshtml");
        }
        ...
        public IActionResult DOCTYPE()
        {
            ...
            string query = "Select Code,Name from DOCTYPE_MAST where CODE in ('RMAR')";
        ...
        public IActionResult ItemMaster()
        {
            ...
            string query = @"SELECT a.CODE AS value, a.name + '|' + ISNULL(b.NAME,'') AS text, a.CODE AS icode, a.name AS [item name],b.NAME AS unit,
                            b.CODE AS ucode, A.SHORTNAME AS [Short Name] FROM item_mast a LEFT JOIN ITEMUNIT_MAST b ON a.UNIT_CODE = b.CODE AND b.comp_code = " + compCode + @"
                            INNER JOIN ITEM_MGROUP c ON a.mGROUP_CODE = c.CODE AND c.COMP_CODE = " + compCode + @" AND c.REPORT_TYPE IN ('Raw','SFG','Waste')
        ...
        [HttpPost]
        public IActionResult SaveData([FromBody] DepartmentReceived model)
        {
            if (model == null)
            {
            ...
            try
            {
                ...
                using (SqlConnection con = _dbConnection.GetErpConnection())
                {
                    ...
                    try
                    {
                        SqlCommand cmd = new SqlCommand("sp_Department_Received", con, trans);
                        ...
                        {
                            SqlCommand cmdItem = new SqlCommand("sp_Department_Received", con, trans);
        ...
        {
            var globalVariable = _globalVariableService.GetGlobalVariables();
            var model = new DepartmentReceived();
            ...
            try
            {
                using (SqlConnection con = _dbConnection.GetErpConnection())
                {
                    SqlCommand cmd = new SqlCommand("sp_Department_Received", con);
                    cmd.CommandType = CommandType.StoredProcedure;
