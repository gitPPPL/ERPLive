...
{
    public class IncommingQCListController : Controller
    {
        ...
        public IncommingQCListController(DataBaseConnection dbConnection, GlobalVariableService globalVariableService,
        ...
        {
            return View("~/Views/QualityControl/Transaction/IncommingQCList/Index.cshtml");
        }
        [HttpGet]
        public JsonResult GetQCIncommingQCEntryList(string searchTerm, int pageNumber = 1, int pageSize = 10)
        {
            ...
            try
            {
                ...
                using (var cmd = new SqlCommand("usp_InsertIncommingQC", con))
                {
            ...
            catch (Exception ex)
            {
                ...
                {
                    success = false,
                    message = "An error occurred while fetching the QC Incoming Entry List.",
                    error = ex.Message
                });
            }
        }
        ...
        public JsonResult Delete(int vNo, string docType)
        {
            try
            {
                ...
                {
                    using (SqlCommand cmd = new SqlCommand("usp_InsertIncommingQC", con))
                    {
                        cmd.CommandType = CommandType.StoredProcedure;
