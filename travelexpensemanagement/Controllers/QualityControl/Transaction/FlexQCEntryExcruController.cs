...
{
    [SessionAuthorize]
    public class FlexQCEntryExcruController : Controller
    {
        ...
        private readonly IFlakesQCEntryExcluRepository _flakesQCEntryExcluRepository;
        public FlexQCEntryExcruController(DataBaseConnection dbConnection, GlobalVariableService globalVariableService,
          ...
          ModuleService.ModuleService moduleService, GlobalValidationdate globalValidationdate , IFlakesQCEntryExcluRepository flakesQCEntryExcluRepository)
        {
            ...
            _flakesQCEntryExcluRepository = flakesQCEntryExcluRepository;
        }
        public IActionResult Index()
        {
            ...
            return View("~/Views/QualityControl/Transaction/FlexQCEntryExcru/Index.cshtml");
        }
        ...
        public JsonResult DDLGridStatus()
        {
            var getdata = _globalVariableService.GetGlobalVariables();
            using (SqlConnection con = _dbConnection.GetErpConnection())
            {
                string query = "select code,name from Status_mast where active=1 and COMP_CODE=" + getdata.PubCompCode + "";

                var DDLGridStatuslist = _dropdownService.GetDropdownList(query);

                return Json(DDLGridStatuslist);
            }

        }

        [HttpPost]
        public IActionResult SavedData([FromBody] FlexQCEntryExcru_Model request)
        {
            ...
            var result = _flakesQCEntryExcluRepository.SubmitRequest(  request.Header, request.Deatils,  action);
